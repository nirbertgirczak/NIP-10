...
class Manager:
    ...
    def generate_apartment_events_report(
        ...
        Args:
        ----
            apartment_key (str): Unique key of the apartment to generate raport for.
            only_unresolved (bool): If True, filters and returns only unresolved events.

        Raises:
        ------
            ValueError: If the provided apartment_keyy does not exist in the system.

        Returns:
        -------
            list[ApartmentEvent]" A list containing all matching apartment events.
    ...
    def get_apartment(self, apartment_key: str) -> Apartment | None:
        ...
        Args:
        ----
            apartment)key (str): Unique string identifier of the apartment.
    ...
    def get_apartment_costs(
        ...
        Args:
        ----
            apartment_key (str): Unique key of the apartment
            year (int): The year for which to calculate the balance. Default is None.
            month (int): The month for which to calculate the balance. Defaults to None.

        Returns:
        -------
            float | None: The annual balance or None if apartment does not exist.

        Example:
        -------
            >>> manager = Manager()
            >>> manager.get_apartment_costs(apartment_key="A1", year=2026)
